...
import { checkInteractionQuality, checkReferentialIntegrity } from './_shared/verify/coverage-checker.js';
...
export default class Interactions extends BaseLlmCommand {
  ...
  static override examples = [
    ...
    '<%= config.bin %> llm interactions --verify',
    '<%= config.bin %> llm interactions --verify --fix',
  ...
  static override flags = {
    ...
    verify: Flags.boolean({
      description: 'Verify existing interactions instead of detecting new ones',
      default: false,
    }),
    fix: Flags.boolean({
  ...
  protected async execute(ctx: LlmContext, flags: Record<string, unknown>): Promise<void> {
    ...
    // Verify mode: run verification instead of detection
    if (flags.verify) {
      this.runInteractionVerify(ctx, flags);
      return;
    }
...
- A realistic cross-process call surface has 3-8 callers per target, not dozens.
  ...
  /**
   * Run interaction quality verification (--verify mode).
   */
  private runInteractionVerify(ctx: LlmContext, flags: Record<string, unknown>): void {
