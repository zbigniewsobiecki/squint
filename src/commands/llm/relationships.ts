...
import type { RelationshipCoverageInfo } from './_shared/coverage.js';
...
export default class Relationships extends BaseLlmCommand {
  static override description = 'Annotate relationships between symbols using an LLM';

  static override examples = [
    '<%= config.bin %> llm relationships',
    '<%= config.bin %> llm relationships --batch-size 10',
    '<%= config.bin %> llm relationships --dry-run',
    '<%= config.bin %> llm relationships --max-iterations 5',
  ];
