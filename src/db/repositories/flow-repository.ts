  ...
  getWithDefinitionSteps(flowId: number): FlowWithDefinitionSteps | null {
    ...
    const stepsStmt = this.db.prepare(`
      SELECT
        ...
        to_m.full_path as toModulePath,
        COALESCE(ra.semantic, i.semantic) as semantic
      ...
      LEFT JOIN interactions i ON i.from_module_id = from_mm.module_id AND i.to_module_id = to_mm.module_id AND i.source IN ('contract-matched', 'llm-inferred')
      WHERE fds.flow_id = ?
