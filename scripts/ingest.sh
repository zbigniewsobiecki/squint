...
./bin/dev.js llm relationships --verify --fix -d $DB 2>&1 | tee -a "$LOG"
wait_for_enter "relationships done"

./bin/dev.js llm modules --model $MODEL -d $DB $DEBUG_FLAGS 2>&1 | tee -a "$LOG"
./bin/dev.js llm modules --verify --fix -d $DB 2>&1 | tee -a "$LOG"
...
./bin/dev.js llm flows --verify --fix -d $DB 2>&1 | tee -a "$LOG"
